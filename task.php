...
if (!$connection) {
    ...
    if (!$action) {
        ...
        if ('add' == $action) {
            ...
            $bulkCompleteTaskIds = $_POST['task_ids'];
            print_r($bulkCompleteTaskIds);
            $_completeTaskIds = join(",", $bulkCompleteTaskIds);
            $bulk_query_update = "UPDATE " . DB_TABLE . " SET complete = 1 WHERE id IN ($_completeTaskIds) ";
            mysqli_query($connection, $bulk_query_update);
            header('location: index.php');
        } else if ('bulk_delete' == $action) {

            $bulkDeleteTaskIds = $_POST['task_ids'];
            print_r($bulkDeleteTaskIds);
            $_bulkDeleteTaskIds = join(",", $bulkDeleteTaskIds);
            $bulk_query_delete = "DELETE FROM " . DB_TABLE . " WHERE id IN ($_bulkDeleteTaskIds) ";
            echo $bulk_query_delete;
            mysqli_query($connection, $bulk_query_delete);
            header('location: index.php');
        }
    }
}
